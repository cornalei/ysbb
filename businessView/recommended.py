import logging
from common.common_fun import Common,TimeoutException,time,TimeoutException
...
class Recommended(Common):
    ...
    # def banner(self):
    #     self.find_element(self.loadpic).click()
    #     return True

    def story(self):
        logging.info('===播放儿童故事===')
        self.find_elements(self.hotuserhead)[1].click()
        try:
    ...
    def focus(self):
        ...
        self.swipeLeft()
        self.find_elements(self.anchor_follow)[0].click()
        self.find_elements(self.anchor_pic)[0].click()
        try:
            self.find_element(self.album).click()
        except TimeoutException:
